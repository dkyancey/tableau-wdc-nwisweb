...
function get_site_info(site){
	var siteInfo = httpGet('https://water.wr.usgs.gov/cgi-bin/get_site_parms.cgi?'+site);
	var x = siteInfo.split('\n');

	y = x[0].split('\t');
	siteparms[site] = "<option value="+y[2]+'">'+y[2]+' - '+y[5]+'</option>';
	sitename[site] = y[1];

	var i;
	for (i=1; i<x.length - 1; i++) {
		y = x[i].split('\t');
		//if (y[2] == undefined) { continue;}
		siteparms[site] += "<option value="+y[2]+'">'+y[2]+' - '+y[5]+'</option>';
	}
	//console.log('siteparms['+site+']='+siteparms[site]);
}
