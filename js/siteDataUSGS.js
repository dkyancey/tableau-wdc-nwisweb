...
(function() {
	...
	myConnector.getSchema = function(schemaCallback) {
		...
		cols.push({id: "URL", alias: "NWISWeb",   dataType: tableau.dataTypeEnum.string });
	...
	myConnector.getData = function(table, doneCallback) {
		...
		$.getJSON(apiCall, function(resp) {
			var ts = resp.value.timeSeries, tableData = [];

			// go thru data to build array keyed on datetime
			var data = new Array();
			var rows = new Array();
			var dt, va, cd, qu, nm, nu, lt, ln, lk;
			// ts.length is number of parameters selected
			for (var i = 0, len = ts.length; i < len; i++) {
				...
				for (var j = 0, l = ts[i].values[j].value.length; j < l; j++) {
					...
					if ( rows.indexOf(dt) < 0) {
						rows.push([dt,nu,nm,lt,ln,lk]);	// start new row
					}
					if ( data[dt] == undefined ){
						data[dt]=[[cd,va,qu]];		// start of data tuples for row
					} else {
						data[dt].push([cd,va,qu]);	// append data tuples to row
					}
				}
			}
			//alert(rows.length);
			...
			for (var i = 0; i < rows.length; i++) {
				...
				//          nu           nm           lt           ln           lk
				tblRow=[dt, rows[i][1], rows[i][2], rows[i][3], rows[i][4], rows[i][5]]; 
